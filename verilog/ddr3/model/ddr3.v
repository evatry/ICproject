...
`include "2048Mb_ddr3_parameters.vh"
    ...
    initial begin
        if (DEBUG) $display ("TCK_MIN = %d", TCK_MIN);
`ifdef sg15E
        if (DEBUG) $display ("sg15E = `sg15E");
`endif  
    
`ifdef sg093                              // sg093  is equivalent to the JEDEC DDR3-2133 (14-14-14) speed bin
        if (DEBUG) $display ("sg093");
`elsif sg15E
        if (DEBUG) $display ("sg15E");
`endif          
    end
            ...
            if (cke) begin
                ...
                case (cmd)
                    LOAD_MODE : begin
                        ...
                        if (DEBUG) $display("LOAD_MODE: bank %d", bank);
                        if (|odt_pipeline)
                            ...
                            case (bank)
                                ...
                                1 : begin
                                    ...
                                    if (!dll_en) begin
                                        ...
                                        if (DEBUG && check_strict_mrbits) $display ("%m: at time %t WARNING: %s %d DLL off mode is not fully modeled", $time, cmd_string[cmd], bank);
